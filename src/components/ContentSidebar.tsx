    ...
    async function fetchRelatedContent() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Degrade to an empty list. This used to fall back to hardcoded
          // "demo" rows attributing invented songs and poems to real, named
          // people (Ilaiyaraaja, A.R. Rahman, Vairamuthu, Kannadasan) and to
          // political figures — shipped to visitors on any API hiccup. False
          // attribution and an apolitical-brand breach are far worse than an
          // empty sidebar, which the component already renders gracefully.
          setRelatedContent([]);
        }
      } catch (error) {
