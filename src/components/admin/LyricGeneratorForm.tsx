    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="space-y-4 rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900/40">
        ...
        <div className="space-y-2">
          ...
          {emotions.length < MAX_EMOTIONS && (
            <button type="button" onClick={addEmotion} className="flex items-center gap-1 text-xs font-medium text-purple-600 hover:text-purple-700 dark:text-purple-300">
              <Plus className="h-3.5 w-3.5" /> Add emotion
            </button>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Register</label>
            <select value={register} onChange={(e) => setRegister(e.target.value as LyricRegister)} className={`mt-1 ${inputCls}`}>
              {REGISTERS.map((r) => (
          ...
          <div>
            ...
            <select value={voice} onChange={(e) => setVoice(e.target.value)} className={`mt-1 ${inputCls}`}>
        ...
        <div className="grid grid-cols-2 items-end gap-3">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Charanams</label>
            <input type="number" min={1} max={5} value={charanams} onChange={(e) => setCharanams(Math.max(1, Math.min(5, Number(e.target.value) || 1)))} className={`mt-1 ${inputCls}`} />
          </div>
        ...
        <div>
          ...
          <input value={seedWords} onChange={(e) => setSeedWords(e.target.value)} placeholder="மண்வாசம், குருவி, வயல்" className={`mt-1 ${inputCls}`} />
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Notes <span className="text-xs font-normal text-gray-400">(optional)</span></label>
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} maxLength={1000} placeholder="Any extra guidance for the lyric" className={`mt-1 ${inputCls}`} />
        </div>
        ...
        <button type="button" onClick={handleSubmit} disabled={!canSubmit} className="flex w-full items-center justify-center gap-2 rounded-lg bg-purple-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-purple-700 disabled:cursor-not-allowed disabled:opacity-50">
          <Sparkles className="h-4 w-4" /> {loading ? 'Generating…' : 'Generate lyrics'}
        </button>
        {error && (
