/**
 * SUNO critic — an LLM pass that predicts SEMANTIC failure modes the
 * deterministic pre-flight linter (src/lib/suno-preflight.ts) can't catch:
 * style↔lyric mood mismatch, unsingable phrasing, ambiguous prompts,
 * pronunciation/language risk. Run BEFORE spending a SUNO credit. Mirrors the
...
export const sunoCritiqueSchema = z.object({
  ...
  issues: z
    ...
    .max(8)
    .describe('Concrete, SUNO-specific risks — empty if genuinely clean.'),
});
export type SunoCritique = z.infer<typeof sunoCritiqueSchema>;

const jsonSchema = z.toJSONSchema(sunoCritiqueSchema) as Record<string, unknown>;

const SYSTEM_PROMPT = `You are an expert at Google/SUNO-style AI music generation, specializing in Tamil songs.
The user gives you a SUNO "style of music" prompt and the song lyrics. SUNO has NO API and every generation costs a paid credit, so your job is to predict — BEFORE they generate — the SEMANTIC problems a rule-based linter cannot see:
...
export async function critiqueSuno(
  ...
  try {
    res = await client.messages.create(
      {
        ...
        tools: [
          {
            name: TOOL_NAME,
            description: 'Submit the SUNO pre-flight critique.',
    ...
    console.error('[ai/sunoCritic] Anthropic call failed:', err instanceof Error ? err.message : String(err));
  ...
  const parsed = sunoCritiqueSchema.safeParse(toolUse.input);
  if (!parsed.success) {
    console.error('[ai/sunoCritic] schema validation failed:', parsed.error.message);
