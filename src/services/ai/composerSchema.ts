...
export const composerAnalysisSchema = z.object({
  ...
  reel: reelSchema,
  // COMPUTED post-hoc by the composer's raga/scale guard — NOT generated by the
  // model (it's stripped from the tool input_schema below). Human-readable notes
  // about any tonal conflicts (e.g. a Dorian key under a major-pentatonic raga)
  // that were detected and corrected before the brief was returned.
  musical_consistency: z.array(z.string()).optional().default([]),
});
 ...
 *
 * `musical_consistency` is dropped: it's computed by the raga/scale guard AFTER
 * the model call, so the model must never be asked to produce it.
 */
export const composerAnalysisJsonSchema = (() => {
  const schema = z.toJSONSchema(composerAnalysisSchema) as {
    properties?: Record<string, unknown>;
    required?: string[];
  };
  if (schema.properties) delete schema.properties.musical_consistency;
  if (Array.isArray(schema.required)) {
    schema.required = schema.required.filter((k) => k !== 'musical_consistency');
  }
