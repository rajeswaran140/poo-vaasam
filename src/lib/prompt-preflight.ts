/**
 * prompt pre-flight linter — validate a composer-generated style prompt +
 * lyrics BEFORE spending a credit. the generator has no API, so every generation costs a
...
export interface PreflightInput {
  /** The "Style of Music" prompt paragraph — must NOT contain lyrics. */
  style: string;
...
/** generator field limits (tunable; based on the custom-mode "Style of Music" box). */
export const PROMPT_LIMITS = {
  STYLE_MAX: 1000, // hard cap — the generator's style box truncates beyond ~1000 chars
  STYLE_SOFT: 850, // warn only as it APPROACHES the cap (composer prompts run rich by design, and current generators accept them)
  LYRICS_MAX_CHARS: 5000, // the generator truncates very long lyrics in a single render
...
const TAMIL = /[஀-௿]/;
// Common emoji / pictographic ranges (the generator may try to "sing" them).
const EMOJI =
  /[\u{1F000}-\u{1FAFF}\u{2600}-\u{27BF}\u{2B00}-\u{2BFF}\u{2190}-\u{21FF}\u{FE0F}\u{2764}]/u;

/** Genre pairs that confuse the generator when combined in one style prompt. */
...
// A good style prompt names these concrete elements.
const CONCRETE = {
...
/**
 * Lint a style prompt + lyrics. Returns findings + a readiness verdict.
 * `ready` is false when any `error` is present (would likely waste the credit).
 */
export function preflightPrompt(input: PreflightInput): PreflightResult {
  ...
  if (!style) {
    ...
    if (style.length > PROMPT_LIMITS.STYLE_MAX) {
      add('style', 'error', 'STYLE_TOO_LONG', `Style is ${style.length} chars — over the ${PROMPT_LIMITS.STYLE_MAX}-char cap; the tail is dropped.`, 'Trim to the essentials (genre, mood, tempo, key instruments, vocal).');
    } else if (style.length > PROMPT_LIMITS.STYLE_SOFT) {
      add('style', 'warning', 'STYLE_LONG', `Style is ${style.length} chars — approaching the ~${PROMPT_LIMITS.STYLE_MAX}-char limit; could clip.`, 'Trim if easy, but rich prompts are fine on most current versions.');
    }
  ...
  if (!lyrics) {
    ...
    if (lyrics.length > PROMPT_LIMITS.LYRICS_MAX_CHARS) {
    ...
    } else if (sungLines.length > PROMPT_LIMITS.LYRICS_SOFT_LINES) {
    ...
    if (input.targetSeconds && input.targetSeconds > 0) {
      const estSeconds = sungLines.length * PROMPT_LIMITS.SECONDS_PER_LINE;
