...
it('reports a processing job', async () => {
  mockGet.mockResolvedValueOnce({ id: 'compose_1', status: 'processing', result: null, error: null });
  const res = await get('compose_1');
...
it('returns the brief when done', async () => {
  mockGet.mockResolvedValueOnce({ id: 'compose_1', status: 'done', result: { emotion: 'காதல்' }, error: null });
  const res = await get('compose_1');
  const body = await res.json();
...
it('surfaces a structured error', async () => {
  mockGet.mockResolvedValueOnce({ id: 'compose_1', status: 'error', result: null, error: { code: 'rate_limit', message: 'Try again' } });
  const res = await get('compose_1');
  const body = await res.json();
