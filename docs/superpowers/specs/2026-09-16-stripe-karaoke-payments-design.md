...
| S3 | Refunds | **A button in the admin panel, behind a confirm step** | Stripe dashboard only; no refund path |
...
| S6 | API key | **A restricted key: Checkout Sessions, Refunds, webhooks only** | A full secret key |
...
This integration needs three capabilities: create a Checkout Session, create a Refund, and verify webhooks. It never needs Customers, Payouts, Balance, Products or Invoices.

The account is shared with Mobily and the parent company. A full secret key leaked from an internet-facing app would reach **every entity's** money; a restricted key reaches this integration's three operations.
...
Handles `checkout.session.completed` only in this phase. Everything else is acknowledged with 200 and ignored — on a shared account this endpoint will receive events that belong to other entities, and it must ignore them silently rather than error.

### `src/app/(admin)/admin/karaoke/page.tsx` + routes (new)

The order list. Approve (with the S4 confirm step) and Refund (with a confirm naming buyer and amount).
...
| Refund fails | Order keeps its state; error surfaced; no silent "refunded" |
...
1. Generate a **restricted key** for Tamilagaval under the TechSynergy account, scoped to Checkout Sessions (write), Refunds (write), and webhook signature verification.
2. `aws ssm put-parameter --type SecureString --name /amplify/d3rkmepk4popv0/master/STRIPE_SECRET_KEY`
3. After the route is deployed, create the webhook endpoint in Stripe pointing at `https://tamilagaval.com/api/stripe/webhook`, subscribed to `checkout.session.completed` only.
